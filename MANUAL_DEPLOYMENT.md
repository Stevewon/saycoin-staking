...
#### 2-2. 데이터베이스 생성
- **Database name**: `saycoin-staking-production`
...
1. D1 데이터베이스 페이지: `saycoin-staking-production` 클릭
2. **Console** 탭 클릭
...
#### 7-2. 기능 테스트
1. **사용자 페이지**: https://saycoin-staking.pages.dev
   - ✅ 회원가입 테스트
   - ✅ 로그인 테스트
   - ✅ 스테이킹 신청 테스트
   - ✅ 잔액 확인
...
2. **관리자 페이지**: https://saycoin-staking.pages.dev/admin
...
```bash
# 회원가입 API
curl -X POST https://saycoin-staking.pages.dev/api/auth/register \
...
# 로그인 API
curl -X POST https://saycoin-staking.pages.dev/api/auth/login \
  -H "Content-Type: application/json" \
...
위 단계를 모두 완료하면 **SAYCOIN STAKING 플랫폼**이 정상적으로 운영됩니다!
